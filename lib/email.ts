...
export async function sendPasswordResetEmail(email: string, otpCode: string, resetUrl: string) {
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #222; max-width: 560px;">
      <h2 style="font-weight: 400; color: #7B2D42;">Reset your Seliora password</h2>
  ...
  if (resendApiKey) {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: resendFrom,
        to: [email],
        subject: 'Reset your Seliora password',
        text: textContent,
        html: htmlContent,
      }),
    });
