...
const CARD_GLOW: Record<string, string> = {
  ...
  'Sapphire': 'shadow-pink-500/40',
  'Emerald':     'shadow-emerald-500/40',
  'Pink Quartz':        'shadow-red-500/40',
  'Ruby Red ':    'shadow-purple-500/40',
};
...
const CARD_DESCRIPTIONS: Record<string, string> = {
  ...
  'Emerald': 'This category includes products ranging from ₹999(+) – ₹1499.',
  'Pink Quartz': 'This category includes products ranging from ₹1499(+) – ₹2499.',
  'Ruby Red': 'This category includes products ranging from ₹2499(+) – ₹3999.',
};
...
function MiniLoyaltyCard({ cardType, onInfoClick }: { cardType: string; onInfoClick?: (cardType: string) => void }) {
  ...
  return (
    ...
    >
      {/* Top-right logo for every mini card (moved slightly left so it doesn't cover the info button) */}
...
export default function ProfilePage() {
  ...
  const bestCount = Math.max(...CARD_TYPES.map(type => cardTypeCounts[type] || 0), 0);
  ...
  const selectedCardCount = selectedCardType ? (cardTypeCounts[selectedCardType] || 0) : 0;
    ...
    <div className="min-h-screen bg-[#0a0a0a] pt-16">
      ...
      <section className="bg-black py-12">
        <div className="max-w-2xl mx-auto px-4 text-center">
          <div className="w-20 h-20 bg-[#7B2D42] flex items-center justify-center text-white text-3xl font-bold mx-auto mb-4">
      ...
      <section className="py-10">
        <div className="max-w-4xl mx-auto px-4 space-y-8">
          ...
          {/* Loyalty Card (spend-based tier) */}
          {loyalty && (
            <div className="relative">
              <p className="text-[10px] tracking-[0.4em] uppercase text-[#C8A96E] mb-4">Your Loyalty Card</p>

              {/* Top-right logo for loyalty section */}
          ...
          <div>
            {/* <div className="flex items-center justify-between mb-4">
              <p className="text-[10px] tracking-[0.4em] uppercase text-[#C8A96E]">Collected Cards</p>
              <span className="text-[10px] text-gray-500 tracking-widest">{cardCount} / 50</span>
            </div> */}

            {/* Progress bar */}
            <div className="h-1 bg-white/10 mb-5 overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-[#7B2D42] to-[#C8A96E] transition-all duration-700"
                style={{ width: `${Math.min((bestCount / 10) * 100, 100)}%` }}
              />
            </div>
            ...
            {earnedGiftTypes.length > 0 && (
              <div className="mb-5 px-4 py-4 bg-gradient-to-r from-amber-900/30 to-yellow-900/20 border border-amber-500/40 text-center">
                ...
                <p className="text-amber-500/70 text-[10px] mt-3 tracking-widest">
                  Our team will reach out to you soon with your reward{earnedGiftTypes.length > 1 ? 's' : ''}.
                </p>
              </div>
            )}
                ...
                <div className="w-full max-w-md border border-[#C8A96E]/30 bg-[#111111] p-6 shadow-2xl relative">
                  ...
                  <h2 className="text-2xl font-serif text-white mb-1">
                    ...
                    {CARD_RANK_LABEL[selectedCardType] && (
                      <span className="text-xs text-gray-500 font-sans ml-2">({CARD_RANK_LABEL[selectedCardType]})</span>
                    )}
                  </h2>
                  <p className="text-sm text-[#C8A96E]/80 leading-relaxed mb-3">
                    {CARD_DESCRIPTIONS[selectedCardType]}
                  </p>
                  <p className="text-sm text-gray-400 leading-relaxed">
                    You currently have {selectedCardCount}/10 of this card. Collect 10 cards of the same type to unlock a gift from our team.
                  </p>
                  {cards.filter((c: any) => c.cardType === selectedCardType).length >= 10 && (() => {
            ...
            {cards.length === 0 && (
              <p className="text-center text-gray-600 text-xs mt-4">Shop with us to earn loyalty cards from our team</p>
            )}
          </div>

          {/* Quick links */}
          <div className="bg-[#111111] border border-white/10 p-4">
            <h2 className="text-[10px] tracking-[0.4em] uppercase text-gray-400 mb-4">Quick Links</h2>
            <Link href="/orders" className="flex items-center gap-3 py-3 px-2 hover:bg-white/5 transition-colors">
